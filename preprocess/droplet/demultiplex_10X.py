...
parser.add_argument('--whitelist', action='store_const', dest='mode',
        const='whitelist',
        help='Identify correct cell barcodes')
parser.add_argument('--extract', action='store_const', dest='mode',
        const='extract',
        help='Extract barcdoes and UMIs and add to read names')
parser.add_argument('-i', action='store', dest='input_path',
        help='path for where fastqs loacate')
parser.add_argument('--cell_num', action='store', dest='cell_num_file',
...
def load_cell_count_hash():
    if not os.path.isfile(paras.cell_num_file):
        return {}
    cell_count_h = {}
...
def whitelist(sample_id, cell_num):
    # CCCCCCCCCCCCCCCCNNNNNNNNNN for 10X V2 toolkit (16+10)
...
def extract(sample_id):
    ...
    --bc-pattern=CCCCCCCCCCCCCCCCNNNNNNNNNN \
    --stdin %s_1.fastq.gz \
    --stdout %s_1.extracted.fastq.gz \
    --read2-in %s_2.fastq.gz \
    --read2-out=%s_2.extracted.fastq.gz \
    --filter-cell-barcode \
    --error-correct-cell \
...
if __name__ == '__main__':
    if paras.mode == "whitelist":
        ...
        for fq1, fq2 in load_fqs():
            ...
            cell_num = cell_count_h.get(sample_id, '4000')
